<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center">
            ...
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
              Enfermeros por Centro de Vacunación
            </h2>
        </div>
    </x-slot>
  ...
  <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
    ...
    Enfermero : <select name="enfermeroBuscado" id="enfermeroBuscado" required>
        <option value=''>Seleccione una Opción</option>
    @foreach ($enfermeros as $enfermero)
        <option value="{{$enfermero->Id}}">{{$enfermero->apelnom}}</option>
    @endforeach

    </select>

    <a href="{{ route('centros.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Buscar</a>
    <br><br>
